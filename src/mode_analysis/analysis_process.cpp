...
bool analysis_cpp::removeVarFromNull(int& index){
    /*
    std::unordered_map < std::string, int >::iterator got = null_index.find(toRandomPeak[v[r]]);
    null_id.erase(null_id.begin() + got->second);
    null_chr.erase(null_chr.begin() + got->second);
    null_start.erase(null_start.begin() + got->second);
    null_end.erase(null_end.begin() + got->second);
    null_maf.erase(null_maf.begin() + got->second);
    upstream_distance.erase(upstream_distance.begin() + got->second);
    downstream_distance.erase(downstream_distance.begin() + got->second);
    nominal.erase(nominal.begin() + got->second);
    
        Each variant kept, get the index from index_dico and remove it from:
        - null_id 
        - null_chr 
        - null_start 
        - null_end
        ...
        - downstream_distance
        - nominal 
        Check then that the size of all of these are the same and continue.
    */
    null_id.erase(null_id.begin() + index);
    null_chr.erase(null_chr.begin() + index);
    null_start.erase(null_start.begin() + index);
    null_end.erase(null_end.begin() + index);
    null_maf.erase(null_maf.begin() + index);
    upstream_distance.erase(upstream_distance.begin() + index);
    downstream_distance.erase(downstream_distance.begin() + index);
    nominal.erase(nominal.begin() + index); 

    if(null_id.size() == null_chr.size() && null_id.size() == null_start.size() && null_id.size() == null_end.size() && null_id.size() == null_maf.size() && null_id.size() == upstream_distance.size() && null_id.size() == downstream_distance.size() && nominal.size() == null_id.size()){
        return true; 
    }else{
        return false;
    }
...
void analysis_cpp::createNullDistribution(string fout){
   ...
   for(int i=0; i<qtl_id.size(); i++){
        ...
        std::unordered_map < std::string, int > null_index;
